...
class Event(Base):
    ...
    time = Column(String, nullable=False)  # Human-readable time (e.g., "2:00 PM - 4:00 PM")
    location = Column(String, nullable=False, index=True)
    category = Column(String, nullable=False, index=True)  # social, educational, fitness, entertainment, volunteer
    capacity = Column(Integer, nullable=False)
    registered = Column(Integer, default=0)
    is_registered = Column(Boolean, default=False)  # User's registration status (per-user in future)
